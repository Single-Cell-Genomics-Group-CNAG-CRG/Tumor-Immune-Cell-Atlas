...
RCTD deconvolution
```{r}
myRCTD <- "{an_oro}/{robj_dir}/RCTDobj_{sample_id}_{clust_vr}.rds" %>%
  glue::glue() %>%
  here::here() %>%
  readRDS(file = .)
```
...
"{an_oro}/{plt_dir}/cor_grid_RCTD-SPOTlight_{sample_id}_{clust_vr}.pdf" %>%
  glue::glue() %>%
  here::here() %>%
  cowplot::save_plot(
    filename = .,
...
"{an_oro}/{plt_dir}/tissue_grid_RCTD-SPOTlight_{sample_id}_{clust_vr}.pdf" %>%
  glue::glue() %>%
  here::here() %>%
  cowplot::save_plot(
    filename = .,
