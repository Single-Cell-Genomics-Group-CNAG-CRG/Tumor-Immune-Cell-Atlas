...
```{r}
dir.create(path = sprintf("%s/%s", an_oro, robj_dir), 
           showWarnings = FALSE, 
           recursive = TRUE)

dir.create(path = sprintf("%s/%s", an_oro, plt_dir),
           showWarnings = FALSE, 
           recursive = TRUE)
```
...
st_ls <- lapply(id_ls, function(id) {
  print(id)
  tmp_se <- Seurat::Load10X_Spatial(data.dir = sprintf("data/australia_hn/%s/", id),
...
## QC
We start by looking at the number of reads, genes, and mitochondrial per spot to check how this tissue looks.
...
lapply(names(st_ls), function(id) {
  ...
  cowplot::save_plot(filename = sprintf("%s/%s/QC_histogram_%s.pdf",
                                        an_oro, plt_dir, id),
                     plot = hist_qc,
                     base_height = 9,
                     base_width = 16)
...
lapply(names(st_ls), function(id) {
  ...
  cowplot::save_plot(filename = sprintf("%s/%s/QC_spatial_%s.pdf",
                                      an_oro, plt_dir, id),
                   plot = sp_qc,
                   base_height = 12,
                   base_width = 12)
...
lapply(names(st_ls), function(id) {
  ...
  cowplot::save_plot(filename = sprintf("%s/%s/QC_spatial_subset_%s.pdf",
                                      an_oro, plt_dir, id),
                   plot = sp_qc,
                   base_height = 12,
                   base_width = 12)
...
## Normalization
Once we've subseted the data to remove low quality spots we will scale, normalize and cluster the data.
```{r}
for (id in names(st_ls)) {
  print(id)
  ...
  st_ls[[id]] <- Seurat::FindClusters(st_ls[[id]], verbose = TRUE, resolution = c(0.1, 0.25, 0.5, 0.75, 1))
...
```{r}
saveRDS(object = st_ls,
        file = sprintf("%s/%s/processed_st_ls_oropharyngeal.RDS",
                       an_oro, robj_dir))
```
...
lapply(names(st_ls), function(id) {
  ...
  cowplot::save_plot(filename = sprintf("%s/%s/Cluster_spatial_%s.pdf",
                                      an_oro, plt_dir, id),
                   plot = sp_clust,
                   base_height = 9,
                   base_width = 15)
