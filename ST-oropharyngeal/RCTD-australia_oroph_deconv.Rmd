...
## Introduction
In this R markdo9wn document we will map immune cells onto the Oropharyngeal cancer tissue using the RCTD  tool.
...
stdir <- "{an_oro}/{robj_dir}/{sample_id}/RCTD_data/spatial" %>%
  glue::glue() %>%
  here::here()
...
"{refdir}/cell_type_dict.csv" %>%
  glue::glue() %>%
  here::here() %>%
  readr::write_csv(x = sc_ls[[2]], file = .)
...
image_id <- names(se_obj@images)
se_obj@images[[image_id]]@coordinates %>%
...
Save RCTD resulting
```{r}
"{an_oro}/{robj_dir}/RCTDobj_{sample_id}_{clust_vr}.rds" %>%
  glue::glue() %>%
  here::here() %>%
